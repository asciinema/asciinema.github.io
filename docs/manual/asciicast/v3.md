...
* output (code `o`),
* of text `Hello world`,
* which happened `1.001` sec after the previous event.

!!! note

    The official asciinema CLI rounds intervals to millisecond precision (3
    decimal places). Higher precision is allowed but rarely beneficial and
    produces noisier output. When rounding intervals to fewer than 6 decimal
    places (microseconds), naively rounding each interval independently can
    accumulate significant timing drift over long recordings. Implementers
    are advised to use error diffusion, a.k.a. Bresenham's algorithm
    (carrying over the rounding error to the next interval), to keep the
    sum of written intervals close to the actual elapsed time.
...
2025-09-10 - Initial version
2025-10-20 - Changed time precision in event examples from microseconds to milliseconds
